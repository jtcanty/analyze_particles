...
function [traces_out,trace_id,sig_all] = PR_utrack_1color_v3(inpath,pixsize)

%load input data direct from specified path
in = load (inpath);

%% Step One: Break trace into separate spots
...
%output: aligned and centered traces in traces cellarray
...
%% Step Three: Review each trace with user input
